...
- Collection section: `Full`, `Triage`, and `Custom` profiles are exposed in the UI. The Collection tab presents the Windows collection surfaces listed above, with available live collectors for event logs, registry, browser artifacts, Jump Lists, SRUM, `$MFT`, `$LogFile`, INDX records, and `$UsnJrnl`.
- Collection section: `Full`, `Triage`, and `Custom` profiles are exposed in the UI. The Collection tab presents the Windows collection surfaces listed above, with available live collectors for event logs, registry, Prefetch, browser artifacts, Jump Lists, SRUM, `$MFT`, `$LogFile`, INDX records, and `$UsnJrnl`.
...
- `src/collections/windows/` -> live Windows collector implementations for browser artifacts, EVTX, Jump Lists, Prefetch, registry, `$MFT`, `$LogFile`, INDX records, SRUM, and `$UsnJrnl`
