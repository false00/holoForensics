...
- Reuses the archive workflow's shared VSS snapshot when Registry is collected with USN, EVTX, MFT, `$LogFile`, INDX, SRUM, Browser Artifacts, Jump Lists, or LNK Files for the same volume, so the collectors read the same point-in-time volume state.
