...
- `vss`: default. Creates or reuses a `Win32_ShadowCopy`, opens the snapshot device without a trailing slash, parses NTFS metadata, and extracts `$MFT` from the point-in-time snapshot.
