...
const MFT_COLLECTION_SCHEMA: &str = "windows_mft_collection_v1";
const MFT_COLLECTOR_NAME: &str = "windows_mft";

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, ValueEnum)]
#[serde(rename_all = "snake_case")]
pub enum MftAcquisitionMode {
    Vss,
    Raw,
}

impl MftAcquisitionMode {
...
pub struct MftCollectCli {
    ...
    #[arg(
        long = "all-volumes",
        help = "Collect $MFT from all detected NTFS volumes"
    )]
    pub all_volumes: bool,

    #[arg(long, value_enum, default_value_t = MftAcquisitionMode::Vss, help = "Acquisition mode; vss is the default")]
    pub mode: MftAcquisitionMode,

    #[arg(
        long = "out-dir",
        help = "Output root directory for collected $MFT artifacts"
    )]
    pub out_dir: PathBuf,

    #[arg(
        long,
        help = "Optional collection manifest path for single-volume collection; defaults to <out-dir>/$metadata/collectors/<volume>/windows_mft/manifest.json"
    )]
    pub manifest: Option<PathBuf>,

    #[arg(
        long = "collection-log",
        help = "Optional collection log path for single-volume collection; defaults to <out-dir>/$metadata/collectors/<volume>/windows_mft/collection.log"
    )]
...
struct ValidationMetadata {
    record_zero_signature: String,
    sampled_records: usize,
    sampled_file_signatures: usize,
    sampled_baad_signatures: usize,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct MftCollectionManifest {
    ...
    mft_real_size: u64,
    data_runs: Vec<MftDataRunMetadata>,
...
pub fn run(args: &MftCollectCli) -> Result<()> {
    ...
    if volumes.is_empty() {
        bail!("no NTFS volumes were found for $MFT collection");
    }
    ...
    for volume in volumes {
        let summary = collect(&MftCollectRequest {
            volume,
...
pub fn collect(request: &MftCollectRequest) -> Result<MftCollectSummary> {
    let mut reporter = |_| {};
    collect_with_progress(request, &mut reporter)
}

pub fn collect_with_progress(
    request: &MftCollectRequest,
    reporter: &mut dyn FnMut(MftProgress),
) -> Result<MftCollectSummary> {
    ...
    match request.mode {
        MftAcquisitionMode::Vss => {
            ...
            let delete_result = vss::delete_shadow_copy(&shadow_copy.id)
                .with_context(|| format!("delete MFT shadow copy {}", shadow_copy.id));
            match (result, delete_result) {
                ...
                (Err(error), Err(delete_error)) => Err(error.context(format!(
                    "also failed to delete MFT shadow copy {}: {delete_error:#}",
                    shadow_copy.id
                ))),
            }
        }
        MftAcquisitionMode::Raw => collect_from_device_paths(
            request,
            &volume,
            &paths,
...
pub fn collect_with_progress_using_shadow_copy(
    ...
    validate_request(request)?;
    if !matches!(request.mode, MftAcquisitionMode::Vss) {
        bail!("shared shadow-copy MFT collection requires --mode vss");
    }
    let volume = usn_journal::normalize_volume(&request.volume)?;
    let paths = output_paths(request, &volume)?;
    let device_paths = shadow_copy_raw_device_paths(&shadow_copy.device_object)?;
...
pub fn default_manifest_path(output_root: &Path, volume: &str) -> Result<PathBuf> {
    collection_metadata::collector_manifest_path(
        output_root,
        volume,
        collection_metadata::WINDOWS_MFT_COLLECTOR,
    )
}
...
fn validate_request(request: &MftCollectRequest) -> Result<()> {
...
fn output_paths(request: &MftCollectRequest, volume: &str) -> Result<OutputPaths> {
    let artifact_archive_path = artifact_archive_path(volume)?;
    let artifact_path = request.out_dir.join(&artifact_archive_path);
    let sha256_path = artifact_path.with_file_name("$MFT.bin.sha256");
...
fn collect_from_device_paths(
    ...
    if request.mode == MftAcquisitionMode::Raw {
        warnings.push(
    ...
    for device_path in device_paths {
        ...
        match extract_mft_from_device(device_path, &paths.artifact_path, reporter) {
            Ok(result) => {
    ...
    let extraction = extraction.ok_or_else(|| {
        anyhow!(
            "all $MFT raw device access attempts failed: {}",
            failures.join("; ")
        )
    })?;
    ...
    let sha256_archive_path = normalize_archive_path_string(
        &paths
            .artifact_archive_path
            .with_file_name("$MFT.bin.sha256"),
    );
    let manifest = MftCollectionManifest {
        metadata_schema: MFT_COLLECTION_SCHEMA.to_string(),
        artifact_type: "windows_mft_collection".to_string(),
        artifact_name: "$MFT".to_string(),
        ...
        collector: CollectorMetadata {
            name: MFT_COLLECTOR_NAME.to_string(),
        ...
        mft_real_size: extraction.mft_real_size,
    ...
    reporter(MftProgress {
        progress_value: 1.0,
        detail: format!("Collected $MFT from {volume}."),
        progress_text: format_bytes(extraction.bytes_written),
    });

    Ok(MftCollectSummary {
        volume: volume.to_string(),
...
fn extract_mft_from_device(
    device_path: &str,
    output_path: &Path,
    reporter: &mut dyn FnMut(MftProgress),
) -> Result<MftExtractionResult> {
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("create MFT output directory {}", parent.display()))?;
    }
    ...
    reporter(MftProgress {
        progress_value: 0.28,
        detail: "Parsing NTFS metadata and locating $MFT runs.".to_string(),
    ...
    let mft_real_size = data_value.len();
    let data_runs = collect_data_runs(&data_value, boot.cluster_size);

    reporter(MftProgress {
        progress_value: 0.38,
        detail: "Streaming $MFT bytes from raw NTFS runs.".to_string(),
        progress_text: format_bytes_progress(0, Some(mft_real_size)),
    });
    ...
    if bytes_written != mft_real_size {
        bail!(
            "$MFT output size mismatch: wrote {} bytes but expected {}",
            bytes_written,
            mft_real_size
        );
    }

    let validation = validate_output_records(output_path, boot.mft_record_size)?;
    Ok(MftExtractionResult {
    ...
    T: Read + Seek,
{
    ...
    loop {
        let bytes_read = data_value
            .read(fs, &mut buffer)
            .with_context(|| format!("read $MFT bytes into {}", output_path.display()))?;
        if bytes_read == 0 {
            break;
        }
        output
            .write_all(&buffer[..bytes_read])
            .with_context(|| format!("write $MFT bytes into {}", output_path.display()))?;
    ...
    T: Read + Seek,
{
    ...
    if mft_lcn == 0 || mft_record_size == 0 {
        bail!("invalid NTFS $MFT boot metadata");
    }
    Ok(NtfsBootMetadata {
        bytes_per_sector,
        sectors_per_cluster,
...
fn validate_output_records(path: &Path, record_size: u64) -> Result<ValidationMetadata> {
    if record_size == 0 || record_size > 1024 * 1024 {
        bail!("invalid $MFT record size {}", record_size);
    }
    ...
    let mut sampled_file_signatures = 0usize;
    let mut sampled_baad_signatures = 0usize;
    ...
    for index in 0..64usize {
        ...
        let signature = &record[..4.min(record.len())];
        if index == 0 {
            first_signature = String::from_utf8_lossy(signature).to_string();
            if signature != b"FILE" {
                bail!("$MFT record 0 signature is not FILE");
            }
        }
        sampled_records += 1;
        if signature == b"FILE" {
            sampled_file_signatures += 1;
        } else if signature == b"BAAD" {
            sampled_baad_signatures += 1;
        }
    }
    Ok(ValidationMetadata {
        record_zero_signature: first_signature,
        sampled_records,
        sampled_file_signatures,
        sampled_baad_signatures,
    })
}
...
fn write_manifest(path: &Path, manifest: &MftCollectionManifest) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
...
fn write_collection_log(path: &Path, manifest: &MftCollectionManifest) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
    ...
    writeln!(
        writer,
        "mft collection volume={} mode={} source={}",
        manifest.volume, manifest.collection_mode, manifest.source_device
    )?;
    writeln!(
        writer,
        "bytes_written={} sha256={} mft_real_size={}",
        manifest.bytes_written, manifest.sha256, manifest.mft_real_size
    )?;
    writeln!(
        writer,
    ...
    writer.flush().context("flush MFT collection log")
}
...
fn load_existing_summary(
    ...
    let manifest: MftCollectionManifest = serde_json::from_slice(&bytes)
...
#[cfg(target_os = "windows")]
fn relaunch_elevated(request: &MftCollectRequest) -> Result<()> {
    ...
    unsafe { ShellExecuteExW(&mut execute) }.context("launch elevated MFT collector via UAC")?;
    ...
    unsafe { GetExitCodeProcess(execute.hProcess, &mut exit_code) }
        .context("read elevated MFT collector exit code")?;
    let _ = unsafe { CloseHandle(execute.hProcess) };
    if exit_code != 0 {
        bail!("elevated MFT collector exited with status {exit_code}");
    }
    Ok(())
}

#[cfg(not(target_os = "windows"))]
fn relaunch_elevated(_request: &MftCollectRequest) -> Result<()> {
    bail!("MFT elevation relaunch is only available on Windows")
}

fn build_relaunch_parameters(request: &MftCollectRequest) -> String {
    let mut values = vec![
        "collect-mft".to_string(),
        "--volume".to_string(),
...
mod tests {
    ...
    use super::{
        MftAcquisitionMode, artifact_archive_path, decode_record_size, default_collection_log_path,
        default_manifest_path, read_boot_metadata, shadow_copy_raw_device_paths,
    };

    #[test]
    fn default_mft_metadata_paths_live_under_central_collector_root() -> Result<()> {
    ...
    #[test]
    fn mft_artifact_archive_path_uses_volume_root() -> Result<()> {
        assert_eq!(
            artifact_archive_path("c:")?,
            PathBuf::from("C").join("$MFT.bin")
        );
        Ok(())
    }
    ...
    fn mode_manifest_values_are_stable() {
        assert_eq!(MftAcquisitionMode::Vss.manifest_value(), "vss");
        assert_eq!(MftAcquisitionMode::Raw.manifest_value(), "raw");
    }
}
