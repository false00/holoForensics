...
const SRUM_COLLECTION_SCHEMA: &str = "windows_srum_collection_v1";
const SRUM_COLLECTOR_NAME: &str = "windows_srum";

#[derive(Debug, Clone, Args)]
pub struct SrumCollectCli {
    #[arg(long, help = "NTFS volume, for example C:")]
    pub volume: String,

    #[arg(
        long = "out-dir",
        help = "Output root directory for collected SRUM artifacts"
    )]
    pub out_dir: PathBuf,

    #[arg(
        long,
        help = "Optional collection manifest path; defaults to <out-dir>/$metadata/collectors/<volume>/windows_srum/manifest.json"
    )]
    pub manifest: Option<PathBuf>,

    #[arg(
        long = "collection-log",
        help = "Optional collection log path; defaults to <out-dir>/$metadata/collectors/<volume>/windows_srum/collection.log"
    )]
...
#[derive(Debug, Clone)]
pub struct SrumCollectSummary {
    pub volume: String,
    pub output_root: PathBuf,
    pub manifest_path: PathBuf,
    pub collection_log_path: PathBuf,
    pub staged_paths: Vec<PathBuf>,
    pub file_records: Vec<SrumCollectedFile>,
    pub failures: Vec<SrumCollectionFailure>,
}

#[derive(Debug, Clone)]
pub struct SrumProgress {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SrumCollectionFailure {
    pub live_path: String,
    pub vss_path: String,
    pub archive_path: String,
...
struct SrumCollectionManifest {
    ...
    files: Vec<SrumCollectedFile>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    failures: Vec<SrumCollectionFailure>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    warnings: Vec<String>,
}

#[derive(Debug, Clone)]
struct PlannedSrumFile {
    source_path: PathBuf,
    live_path: String,
    archive_path: PathBuf,
}

pub fn run(args: &SrumCollectCli) -> Result<()> {
    let summary = collect(&SrumCollectRequest {
    ...
    println!("Collected {} SRUM files.", summary.file_records.len());
    println!("Failed {} SRUM files.", summary.failures.len());
    println!("Manifest: {}", summary.manifest_path.display());
    println!("Collection log: {}", summary.collection_log_path.display());
    Ok(())
}

pub fn collect(request: &SrumCollectRequest) -> Result<SrumCollectSummary> {
    let mut reporter = |_| {};
    collect_with_progress(request, &mut reporter)
}

pub fn collect_with_progress(
    request: &SrumCollectRequest,
    reporter: &mut dyn FnMut(SrumProgress),
) -> Result<SrumCollectSummary> {
    ...
    let delete_result = vss::delete_shadow_copy(&shadow_copy.id)
        .with_context(|| format!("delete SRUM shadow copy {}", shadow_copy.id));
    match (result, delete_result) {
        ...
        (Err(error), Err(delete_error)) => Err(error.context(format!(
            "also failed to delete SRUM shadow copy {}: {delete_error:#}",
            shadow_copy.id
        ))),
    }
}
...
pub fn default_manifest_path(output_root: &Path, volume: &str) -> Result<PathBuf> {
    collection_metadata::collector_manifest_path(
        output_root,
        volume,
        collection_metadata::WINDOWS_SRUM_COLLECTOR,
    )
}
...
fn validate_request(request: &SrumCollectRequest) -> Result<()> {
...
fn collect_from_shadow_copy(
    ...
    reporter(SrumProgress {
        progress_value: 0.05,
        detail: format!("Enumerating SRUM files on {volume}."),
    ...
    let planned = plan_srum_files(&volume, &source_root)?;
    ...
    for (index, planned_file) in planned.into_iter().enumerate() {
        ...
        reporter(SrumProgress {
            ...
            progress_text: format!("{index} / {total} SRUM"),
        });
        match copy_srum_file(&planned_file, &destination_path) {
            Ok(record) => {
                staged_paths.push(destination_path);
                file_records.push(record);
            }
            Err(error) => failures.push(SrumCollectionFailure {
    ...
    reporter(SrumProgress {
        progress_value: 0.94,
        detail: "Writing SRUM manifest and collection log.".to_string(),
        progress_text: "Manifest".to_string(),
    });
    let end_time = Utc::now();
    let manifest = SrumCollectionManifest {
        metadata_schema: SRUM_COLLECTION_SCHEMA.to_string(),
        artifact_type: "windows_srum_collection".to_string(),
        artifact_name: "Windows SRUM".to_string(),
        volume: volume.clone(),
        ...
        collector: CollectorMetadata {
            name: SRUM_COLLECTOR_NAME.to_string(),
        ...
        source_globs: vec![
            format!("{volume}\\Windows\\System32\\sru\\*"),
            format!("{volume}\\Windows\\System32\\config\\SOFTWARE"),
            format!("{volume}\\Windows\\System32\\config\\SYSTEM"),
        ],
        privileges_enabled,
    ...
    reporter(SrumProgress {
        progress_value: 1.0,
        detail: format!(
            "Copied {} of {} SRUM files from {volume}.",
            file_records.len(),
            total
        ),
...
fn load_existing_summary(
    ...
    let manifest: SrumCollectionManifest = serde_json::from_slice(&bytes)
...
fn plan_srum_files(volume: &str, source_root: &Path) -> Result<Vec<PlannedSrumFile>> {
    let normalized_volume = usn_journal::normalize_volume(volume)?;
    let srum_dir = source_root.join("Windows").join("System32").join("sru");
    let archive_root = volume_archive_root(&normalized_volume)?;
    let live_srum_root = format!("{}\\Windows\\System32\\sru", normalized_volume);
    let mut planned = Vec::new();

    if srum_dir.exists() {
        for entry in fs::read_dir(&srum_dir)
            .with_context(|| format!("read SRUM directory {}", srum_dir.display()))?
        {
            let entry = entry.with_context(|| format!("read entry in {}", srum_dir.display()))?;
            ...
            let file_name = source_path
                .file_name()
                .ok_or_else(|| anyhow::anyhow!("SRUM path has no file name"))?;
            planned.push(PlannedSrumFile {
                source_path: source_path.clone(),
                live_path: format!(r"{live_srum_root}\{}", file_name.to_string_lossy()),
    ...
    let required_srudb_archive_path = archive_root
        .join("Windows")
        .join("System32")
        .join("sru")
        .join("SRUDB.dat");
    if !planned
        .iter()
        .any(|file| file.archive_path == required_srudb_archive_path)
    {
        planned.push(PlannedSrumFile {
            source_path: srum_dir.join("SRUDB.dat"),
            live_path: format!(r"{live_srum_root}\SRUDB.dat"),
            archive_path: required_srudb_archive_path,
        });
    }
    ...
    for hive in ["SOFTWARE", "SYSTEM"] {
        ...
        planned.push(PlannedSrumFile {
            source_path,
            live_path: format!(r"{normalized_volume}\Windows\System32\config\{hive}"),
            archive_path: archive_root
                .join("Windows")
                .join("System32")
                .join("config")
...
fn copy_srum_file(
    planned_file: &PlannedSrumFile,
    destination_path: &Path,
) -> Result<SrumCollectedFile> {
    if let Some(parent) = destination_path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("create SRUM destination directory {}", parent.display()))?;
    }
...
fn write_manifest(path: &Path, manifest: &SrumCollectionManifest) -> Result<()> {
...
fn write_collection_log(path: &Path, manifest: &SrumCollectionManifest) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
    ...
    writeln!(writer, "srum collection volume={}", manifest.volume)?;
    ...
    writer.flush().context("flush SRUM collection log")
}
...
#[cfg(target_os = "windows")]
fn relaunch_elevated(request: &SrumCollectRequest) -> Result<()> {
    ...
    unsafe { ShellExecuteExW(&mut execute) }.context("launch elevated SRUM collector via UAC")?;
    ...
    unsafe { GetExitCodeProcess(execute.hProcess, &mut exit_code) }
        .context("read elevated SRUM collector exit code")?;
    let _ = unsafe { CloseHandle(execute.hProcess) };
    if exit_code != 0 {
        bail!("elevated SRUM collector exited with status {exit_code}");
    }
    Ok(())
}

#[cfg(not(target_os = "windows"))]
fn relaunch_elevated(_request: &SrumCollectRequest) -> Result<()> {
    bail!("SRUM elevation relaunch is only available on Windows")
}

fn build_relaunch_parameters(request: &SrumCollectRequest) -> String {
    let mut values = vec![
        "collect-srum".to_string(),
        "--volume".to_string(),
...
mod tests {
    ...
    use super::{default_collection_log_path, default_manifest_path, plan_srum_files};

    #[test]
    fn default_srum_metadata_paths_live_under_central_collector_root() -> Result<()> {
    ...
    #[test]
    fn plan_srum_files_collects_sru_folder_and_supporting_hives() -> Result<()> {
        let temp = tempdir()?;
        let source_root = temp.path().join("shadow");
        let sru = source_root.join("Windows").join("System32").join("sru");
        let config = source_root.join("Windows").join("System32").join("config");
        fs::create_dir_all(&sru)?;
        fs::create_dir_all(&config)?;
        fs::write(sru.join("SRUDB.dat"), b"database")?;
        fs::write(sru.join("SRU00001.log"), b"log")?;
        fs::write(sru.join("SRU.chk"), b"checkpoint")?;
        fs::write(config.join("SOFTWARE"), b"software")?;
        fs::write(config.join("SYSTEM"), b"system")?;

        let planned = plan_srum_files("c:", &source_root)?;
        ...
        assert!(
            planned
                .iter()
                .any(|file| { file.live_path == r"C:\Windows\System32\sru\SRUDB.dat" })
        );
        Ok(())
    }
}
